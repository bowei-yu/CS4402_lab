...
int main (int argc, char *argv[])
{
   ...
   // call and time evaluate MPI_Sort_direct
	...
	if (rank == 0) {
      ...
      printf("Overall execution time with %d processors = %lf \n", size, overallTime);
	}
...
int MPI_Sort_direct(int n, double * array, int root, MPI_Comm comm){
    ...
    double * scattered_array = (double *) calloc(n/size, sizeof(double));
    
    // scatter array to localArray with double elements
    int rc = MPI_Scatter(array, n/size, MPI_DOUBLE, scattered_array, n/size, MPI_DOUBLE, root, comm);
    if (rc != MPI_SUCCESS) {
    	return rc;
    }
	
    // sort localArray
    bubble_sort(n/size, scattered_array);

    // gather localArray to array with double elements
    rc = MPI_Gather(scattered_array, n/size, MPI_DOUBLE, array, n/size, MPI_DOUBLE, root, comm);
    if (rc != MPI_SUCCESS) {
    	return rc;
    }
    if( rank == 0 ) {
