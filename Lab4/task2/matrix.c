...
int MPI_Prod_matrix(int n, int ** a, int ** b, int ** c, int root, MPI_Comm comm)
{
	...
	MPI_Scatter(&a[0][0], n*n/size, MPI_INT, &local_a[0][0], n*n/size, MPI_INT, root, comm);
	MPI_Bcast(&b[0][0], n*n, MPI_INT, root, comm);

	// calculate local_c = local_a * b
	local_c = pseudo_prod_matrix(n/size, n, n, local_a, trans_matrix(n, n, b));
	
	// gather local_c
	MPI_Gather(&local_c[0][0], n*n/size, MPI_INT, &c[0][0], n*n/size, MPI_INT, root, comm);
